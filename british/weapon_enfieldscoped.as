...
class weapon_enfieldscoped : ScriptBasePlayerWeaponEntity
{
	...
	void PrimaryAttack()
	{
		...
		g_SoundSystem.EmitSoundDyn( self.m_pPlayer.edict(), CHAN_WEAPON, "weapons/ww2projekt/enfield_shoot.wav", 1.0, ATTN_NORM, 0, PITCH_NORM );
	...
	void Reload()
	{
		...
		if( self.m_iClip >= 5 )
			self.DefaultReload( ENFIELDS_MAX_CLIP, ENFIELDS_RELOAD, 3.68, 0 );
		else if( self.m_iClip < 5 )
			self.DefaultReload( ENFIELDS_MAX_CLIP, ENFIELDS_RELOAD_LONG, 5.48, 0 );
	}
